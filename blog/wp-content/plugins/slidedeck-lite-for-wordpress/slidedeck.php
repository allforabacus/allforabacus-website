...
define( 'SLIDEDECK_VERSION',                        '1.4.1' );
...
function slidedeck_admin_init() {
    ...
    if ( slidedeck_is_plugin() ) {
        ...
        wp_register_script( 'slidedeck-admin-js', slidedeck_url( '/slidedeck-admin.js' ), array( 'jquery' ), SLIDEDECK_VERSION );
 ...
 */
function slidedeck_process_slide_content( $content, $editing = false ) {
    global $slidedeck_global_options;
                ...
                if ( is_array( $val1 ) ) {
                    ...
                    foreach ( (array) $val1 as $key2 => $val2 ) {
                        ...
                        switch ( $key2 ) {
                            case "title":
                            case "content":
                                $sanitized = slidedeck_sanitize( $val2 );
                            break;
                        }
...
function slidedeck_shortcode( $atts ) {
    ...
    if ( $id !== false ) {
        return slidedeck_process_template( $id, array( 'width' => $width, 'height' => $height ) );
    } else {
        return "";
    }
}
...
function slidedeck_process_template( $slidedeck_id, $styles = array( 'width' => '100%', 'height' => '300px' ) ) {
    ...
    if ( isset( $slidedeck ) && !empty( $slidedeck ) ) {
                ...
                ob_start();
                    ...
                    foreach ( $slide_nodes as $node => $val ) {
                        ...
                        if( empty( $slide_nodes['image'] ) && $image_skin ){
                            ...
                            include( $skin['templates']['two_column_copy_image']['file'] );
        ...
        $slidedeck_footer_scripts .= '<script type="text/javascript">jQuery( \'#' . $slidedeck_uid . '\' ).slidedeck( ' . $slidedeck_options_json . ' );</script><noscript>Powered by <a href="http://www.slidedeck.com" target="_blank">SlideDeck WordPress Slider</a></noscript>';
 ...
 * @return Rendered SlideDeck markup and JavaScript tag to initialize SlideDeck render
 */
function slidedeck( $slidedeck_id, $styles=array( 'width' => '100%', 'height' => '370px' ) ) {
    
    $slidedeck_id = slidedeck_legacy_id( $slidedeck_id );
    
    // Send the SlideDeck parameters through the output template processor
    $template_str = slidedeck_process_template( $slidedeck_id, $styles );
